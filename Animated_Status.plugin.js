...
class AnimatedStatus {
  ...
  start() {
    if (this.animation.length === 0) {
      BdApi.showToast("Status Animado: Não há nenhum status definido, defina em Configurações.");
    } else {
      this.animationLoop();
    }
  }
  ...
  newEditorRow({ text, emoji_name, emoji_id, timeout } = {}) {
    ...
    const textWidget = hbox.appendChild(GUI.newInput(text, "Texto"));
    ...
    const optTimeoutWidget = hbox.appendChild(GUI.newNumericInput(timeout, this.kMinTimeout, "Tempo"));
  ...
  getSettingsPanel() {
    ...
    settings.appendChild(GUI.newLabel("Duração da etapa (3000: 3 segundos, 3500: 3,5 segundos, ...), substituída por etapas individuais"));
    ...
    const save = actions.appendChild(GUI.setSuggested(GUI.newButton("Salvar")));
    save.onclick = () => {
      try {
      ...
      BdApi.showToast("Configurações salvas!", { type: "success" });
  ...
  strError(req) {
    ...
    if (req.status === 401) return "Token de Autenticação invalido";

    let json = JSON.parse(req.response);
    for (const s of ["errors", "custom_status", "text", "_errors", 0, "message"])
      if ((json === undefined) || ((json = json[s]) === undefined))
        return "Erro desconhecido. reporte em: github.com/Santozrlk/Status-Animado";
  ...
  newNumericInput: (text = "", minimum = 0, placeholder = "") => {
    ...
    out.addEventListener("focusout", () => {
      if (parseInt(out.value) < minimum) {
        ...
        BdApi.showToast(`O valor não deve ser inferior a ${minimum}`, { type: "error" });
      }
    });
    return out;
  },
